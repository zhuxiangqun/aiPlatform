...
| 生成 app 运行时 | builder/app_runtime.py（detect_runtime/launch/health_check/stop/smoke_test/real_tests/auto_repair + _register_smoke_failure + _register_test_failure）+ api/routers/builder.py 端点 /platform/builder/projects/{id}/runtime*（launch/runtime/stop/smoke/real-tests/auto-repair）+ builder_project_service.py（run_tests 升级真实冒烟 + 真实测试 + 自动修复） | ✅ | 生成 app 运行能力（2026-08-27，生成物侧接线收尾）：detect_runtime 扫描生成目录识别入口（FastAPI uvicorn/Flask/Node/静态页 http.server）→ launch 经 daemon_jobs 托管启动（派生端口 18000-18999、127.0.0.1 绑定）→ health_check HTTP 轮询探测（2xx/3xx=up）→ stop kill 会话组；run_tests 的 e2e_smoke 从"目录存在"假通过升级为真实冒烟（启动+健康探测，自动测试闭环）；**real_tests 测试经理真实测试**：递归发现生成物测试用例（backend/tests/ 等任意层级 test_*.py）→ 可写临时目录跑 pytest（装依赖 + PYTHONPATH + conftest）→ test_report（header/meta/test_results/bug_summary，对齐 test_executor；失败分类 env/配置/实现，含 suggested_fix）；**auto_repair 自动修复闭环**：测试失败 → LLM（llm_generate 经 CoreFacade）按测试输出修复生成代码 → 可写临时区验证 → 改进则写回部署目录（_sync_repair_writeback，路径段匹配防逃逸）→ 重跑测试，最多 max_rounds 轮；冒烟失败（launch 失败/健康不通过）→ L2 经验回写（generated-smoke-*）；真实测试失败（断言失败/配置错误/超时）→ L2 经验回写（generated-test-failed，含 suggested_fix）——均与 conformance 拒绝登记同源；生成物适用：**已接线**（生成 app 可运行 + 自动测试 + 测试经理真实测试 + 自动修复 + 失败经验回写） | 已合入 |
