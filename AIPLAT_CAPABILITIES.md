...
| **Knowledge Pipeline 生成物适用性** | 生成 agent 运行时知识检索接入 | ⚠️ | 生成物适用：**待评估**（生成 agent/SKILL.md 是否应检索平台知识库——弱适用，需生成物侧接线评估，2026-08-27 审计标注） | 待评估 |
...
| kb 生成物适用性 | kb | ⚠️ | 生成物适用：待评估（生成应用知识服务接入候选，与 Knowledge Pipeline 一致） | 已评估 |
