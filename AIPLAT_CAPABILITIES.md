...
| 生成物适用性守卫 | scripts/check_generated_artifact_wiring.py（discover_modules/check）+ architecture_guard.sh §97 | ✅ | 每个 governance 能力模块必须：① CAPABILITIES 有条目（含模块路径）② 条目含"生成物"适用性评估声明（适用+接线状态 / 不适用+理由）——CLAUDE.md §23 强制规则；防平台-产物脱节 | 已合入 |
