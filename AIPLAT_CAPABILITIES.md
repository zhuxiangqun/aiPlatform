...
| 生成 app 运行时 | builder/app_runtime.py（detect_runtime/launch/health_check/stop/smoke_test + _register_smoke_failure）+ api/routers/builder.py 端点 /platform/builder/projects/{id}/runtime*（launch/runtime/stop/smoke）+ builder_project_service.py（run_tests 升级真实冒烟） | ✅ | 生成 app 运行能力（2026-08-27，生成物侧接线收尾）：detect_runtime 扫描生成目录识别入口（FastAPI uvicorn/Flask/Node/静态页 http.server）→ launch 经 daemon_jobs 托管启动（派生端口 18000-18999、127.0.0.1 绑定）→ health_check HTTP 轮询探测（2xx/3xx=up）→ stop kill 会话组；run_tests 的 e2e_smoke 从"目录存在"假通过升级为真实冒烟（启动+健康探测，自动测试闭环）；冒烟失败（launch 失败/健康不通过）→ L2 经验回写（generated-smoke-*，与 conformance 拒绝登记同源）；生成物适用：**已接线**（生成 app 可运行 + 自动测试 + 失败经验回写） | 已合入 |
