...
const CanvasInner: React.FC = () => {
  ...
  const _dragRect = useRef<DOMRect | null>(null);
  const onDragEnter = useCallback((e: React.DragEvent) => {
    _dragRect.current = (e.currentTarget as HTMLElement).getBoundingClientRect();
  }, []);
  const onDragOver = useCallback((e: React.DragEvent) => { e.preventDefault(); e.dataTransfer.dropEffect = 'move'; if (_dragRect.current) { setDragPos({ x: e.clientX - _dragRect.current.left, y: e.clientY - _dragRect.current.top }); } }, []);
  ...
  return (<div className="h-[calc(100vh-4rem)] flex">
    ...
    <div className="flex-1 flex flex-col">
      ...
      <div className="flex-1 bg-dark-bg relative" onDragEnter={onDragEnter} onDragOver={onDragOver} onDrop={onDrop} onDragLeave={() => setDragPos(null)} onContextMenu={e => { if ((e.target as HTMLElement).closest('.react-flow__node')) return; e.preventDefault(); if (!isRunning) setCanvasMenu({ x: e.clientX, y: e.clientY }); }} onClick={() => setCanvasMenu(null)}>
