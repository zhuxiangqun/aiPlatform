...
import { Button, Card, CardContent, CardHeader, Input, Modal, Progress, Table, Textarea, toast } from '../../../components/ui';
...
type TabKey = 'collections' | 'documents' | 'query' | 'stats' | 'eval' | 'tools' | 'settings';
...
const KnowledgeBase: React.FC = () => {
  ...
  const [_evalReports, setEvalReports] = useState<any[]>([]);
  ...
  const [aiForm, setAiForm] = useState({ title: '', prompt: '' });
  ...
  const [uploadKind, setUploadKind] = useState<string>('pdf');
  const [urlKind, setUrlKind] = useState<string>('pdf');
  ...
  const runEval = async () => {
    ...
    try {
      ...
      setEvalResult(r); await refreshEvalReports();
      toast.success(`${r.reports || 0} 个报告完成`);
    } catch (e: any) { toast.error(`评估失败：${e?.message || e}`); }
    finally { setEvalLoading(false); }
  };
  const refreshEvalReports = async () => {
    try { const r = await kbApi.listEvalReports(50, 0); setEvalReports(r.items || []); } catch {}
  };

  // ── Tools helpers ──
  const handleAiCreate = async () => {
    if (!aiForm.prompt.trim()) { toast.error('请输入创作要求'); return; }
    setToolLoading(true);
    try {
      const r = await kbApi.createWithAi(aiForm.title || 'AI生成文档', aiForm.prompt, selectedCollection);
      setToolResult(r); toast.success(`已创建：${r.doc_id}`);
    } catch (e: any) { toast.error(`创作失败：${e?.message || e}`); }
    finally { setToolLoading(false); }
  };
  const handleStorageCleanup = async () => {
    if (!confirm('清理所有归档元素和孤立嵌入？')) return;
    setToolLoading(true);
    try { const r = await kbApi.cleanupStorage(); setToolResult(r); toast.success(`清理完成`); }
    catch (e: any) { toast.error(`清理失败：${e?.message || e}`); }
    finally { setToolLoading(false); }
  };
  const refreshStorage = async () => {
    try { const r = await kbApi.getStorageStats(); setStorageStats(r); } catch {}
  };
  const handleContentSave = async (docId: string, content: string) => {
    try { await kbApi.updateDocContent(docId, content); toast.success('已保存'); } catch (e: any) { toast.error(`保存失败：${e?.message || e}`); }
  };
  void handleContentSave; // suppress unused warning
  ...
  const onUpload = async (file: File) => {
    setUploading(true);
    try {
      const res = await kbApi.uploadDocument(selectedCollection, file, uploadKind as any);
      const jobId =
        res?.job?.job_id ||
        res?.job_id ||
  ...
  const onUrlIngest = async () => {
    ...
    try {
      const res = await kbApi.ingestDocumentByUrl(selectedCollection, raw, urlKind as any, 'zh', 60);
  ...
  const startMaterialsConversation = async () => {
    ...
    try {
      ...
      navigate(`/platform/kb/chat/${encodeURIComponent(sid)}`);
    } catch (e: any) {
  ...
  return (
    <div className="space-y-4">
          ...
          <CardContent>
             <div className="flex items-center gap-2 mb-3 flex-wrap">
               <input
                ref={fileInputRef}
                type="file"
                accept=".pdf,.txt,.md,.docx,.doc,.pptx,.ppt,.mp4,.mov,.mkv,.avi,.webm,.m4v"
                className="hidden"
                onChange={(e) => {
                  ...
                  if (f) {
                    ...
                    setUploadKind(ext ? (extMap[ext] || 'pdf') : 'pdf');
                    onUpload(f);
                  }
            ...
            <div className="mb-3 rounded-xl border border-dark-border bg-dark-card p-3">
              <div className="text-sm font-medium text-gray-200 mb-2">URL 导入</div>
              <div className="text-xs text-gray-400 mb-3">
                支持 PDF/文档直链，视频直链或视频平台页面（自动识别类型）。
              </div>
                ...
                <div className="min-w-[320px] flex-1">
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      if (/\.(mp4|mov|mkv|avi|webm|m4v)(\?|$)/.test(v)) setUrlKind('video');
                      else if (/toutiao\.com|douyin\.com|bilibili\.com|youtube\.com|youtu\.be|ixigua\.com/i.test(v)) setUrlKind('video');
                      else setUrlKind('pdf');
                    }}
                    placeholder="https://example.com/file.pdf 或 https://www.toutiao.com/video/..."
                  />
                </div>
      ...
      {tab === 'stats' && (
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <div className="font-semibold text-gray-100">知识库统计</div>
                <div className="text-xs text-gray-400 mt-0.5">Tenant: {tenantId}</div>
              </div>
              <Button variant="secondary" loading={loadingStats} onClick={async () => {
              ...
              <Button variant="secondary" onClick={async () => {
                if (!confirm('重新计算所有文档的 Embedding 向量？这会用当前模型（Jina）重新索引。')) return;
                setLoadingStats(true);
                try {
                  const res = await kbApi.reindex();
                  toast.success(`已重新索引 ${res.count || 0} 个元素`);
                  await kbApi.getStats().then(setStats);
          ...
          <CardContent>
            {!stats && !loadingStats ? (
              <div className="text-center py-12 text-gray-500">
                <div className="text-sm">点击"刷新"加载统计</div>
              </div>
            ) : (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  { label: '文档总数', value: stats?.documents ?? '-', color: 'text-blue-400', bg: 'bg-blue-500/5', icon: '📄' },
                  { label: '文本元素', value: stats?.elements ?? '-', color: 'text-green-400', bg: 'bg-green-500/5', icon: '📝' },
                  { label: '向量索引', value: stats?.embeddings ?? '-', color: 'text-purple-400', bg: 'bg-purple-500/5', icon: '🧬' },
                  { label: '待处理任务', value: stats?.jobs_pending ?? '-', color: stats?.jobs_pending > 0 ? 'text-orange-400' : 'text-gray-400', bg: stats?.jobs_pending > 0 ? 'bg-orange-500/5' : 'bg-dark-hover', icon: '⏳' },
                ].map((item) => (
                  <div key={item.label} className={`rounded-xl border border-dark-border ${item.bg} p-4`}>
                ...
                <div className="col-span-2 md:col-span-4 rounded-xl border border-dark-border bg-dark-bg p-4">
                  <div className="text-xs text-gray-500 mb-2">集合</div>
                  <div className="text-sm text-gray-300">{stats?.collections ?? '-'} 个</div>
                  <div className="text-xs text-gray-600 mt-1">
                    嵌入模型：transform · 检索策略：关键词 + FTS5 + 向量 (RRF融合) · Rerank：Cross-Encoder风格
                  </div>
                </div>
              </div>
            )}
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-2 mb-3">
                <Input label="问题" value={evalForm.question} onChange={e => setEvalForm({...evalForm, question: e.target.value})} placeholder="例如：深度学习是什么" />
                <Input label="标准答案" value={evalForm.ground_truth} onChange={e => setEvalForm({...evalForm, ground_truth: e.target.value})} placeholder="正确的回答内容" />
                <Input label="关联文档ID (逗号分隔)" value={evalForm.doc_ids} onChange={e => setEvalForm({...evalForm, doc_ids: e.target.value})} placeholder="doc_001, doc_002" />
                <Input label="标签 (逗号分隔)" value={evalForm.tags} onChange={e => setEvalForm({...evalForm, tags: e.target.value})} placeholder="ai, basics" />
              </div>
              <div className="flex gap-2">
                ...
                <div className="mt-3 space-y-1 max-h-40 overflow-auto">
                  {evalSamples.map((s: any) => (
                    <div key={s.id} className="flex items-center gap-2 text-xs py-1 px-2 bg-dark-bg rounded">
                      ...
                      <button onClick={() => deleteEvalSample(s.id)} className="text-red-400 hover:text-red-300">✕</button>
                    </div>
                  ))}
                </div>
              )}
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex gap-2 mb-3">
                ...
                <Button variant="secondary" onClick={refreshEvalReports}>刷新报告</Button>
              </div>
              {evalResult && (
                <div className="grid grid-cols-4 gap-2 mb-3">
                  {Object.entries(evalResult.avg_metrics || {}).map(([k, v]) => (
                    <div key={k} className="bg-dark-bg rounded-lg p-3 text-center">
      ...
      {tab === 'tools' && (
        <div className="space-y-4">
          <Card>
            <CardHeader><div className="font-semibold text-gray-100">🤖 AI 创作文档</div></CardHeader>
            <CardContent>
              <div className="space-y-2">
                <Input label="标题" value={aiForm.title} onChange={e => setAiForm({...aiForm, title: e.target.value})} placeholder="文档标题" />
                <Textarea label="创作要求" value={aiForm.prompt} onChange={e => setAiForm({...aiForm, prompt: e.target.value})} rows={3} placeholder="描述你要创建的文档内容..." />
                <Button variant="primary" loading={toolLoading} onClick={handleAiCreate}>生成文档</Button>
                {toolResult?.doc_id && <div className="text-xs text-green-400 mt-2">已创建：{toolResult.doc_id}</div>}
              </div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader><div className="font-semibold text-gray-100">🗄️ 存储管理</div></CardHeader>
            <CardContent>
              <div className="flex gap-2 mb-3">
                <Button variant="secondary" onClick={refreshStorage}>刷新统计</Button>
                <Button variant="danger" loading={toolLoading} onClick={handleStorageCleanup}>清理归档</Button>
              </div>
              {storageStats && (
                <div className="grid grid-cols-3 gap-2 text-xs">
                  {['documents','active','elements','embeddings','archived_elements'].map(k => (
                    <div key={k} className="bg-dark-bg rounded p-2"><span className="text-gray-500">{k}</span><br/><span className="text-gray-200">{storageStats[k] ?? '-'}</span></div>
                  ))}
                  <div className="bg-dark-bg rounded p-2 col-span-3"><span className="text-gray-500">数据库大小</span><br/><span className="text-gray-200">{((storageStats.db_size_bytes || 0) / 1024).toFixed(1)} KB</span></div>
                </div>
              )}
            </CardContent>
          </Card>
