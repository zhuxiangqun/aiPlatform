...
日期：2026-07-18 (updated from 2026-04-16 / 2026-07-13)
...
| Dashboard（总览） | 聚合四层 status/metrics/health；数据来自各层 API；统一计算 overall_status | `DashboardAggregator` 存在；通过 HTTP adapter 转发至各层 | ✅ core/platform/app 均已通过 HTTP adapter 转发 (2026-07-18) |
| Monitoring（采集） | 定时采集 metrics 写入时序存储（InfluxDB 等）；对外提供图表查询 | Prometheus + OpenTelemetry 已集成；`/metrics` 端点可用 | ✅ OpenTelemetry + Prometheus /metrics 已接线 (Phase 0.2)；长期历史存储可选 |
| Alerting（告警） | 规则持久化 + 生命周期 + 去重/抑制 + 通知通道 + 历史查询 | `AlertEngine` 已增加 SQLite 持久化 (2026-07-18)；`EmailNotifier` 已注册为通知通道 | ✅ 持久化已实现；通知通道 EmailNotifier 已接线 |
| Diagnostics（诊断） | 诊断逻辑在各层；management 聚合/标准化输出；支持 trace/logs 链接 | `api/diagnostics.py` + `TraceFlowGraph.tsx` 前端可视化已实现 | ✅ trace_id/span_id 全链路追踪已完成 (Phase 0.2) |
| Config（配置） | 版本控制/发布/回滚；配置落库；对各层下发并记录审计 | YAML 配置已迁移；跨层下发为独立需求 | ⚠️ 配置已迁移到 YAML (config/*.yaml)；跨层下发为 vNext 路线图 |
