...
### 2.7 Learning / Runs / Artifacts（当前实现）

**文档**：[learning.md](./learning.md)

**核心功能（As‑Is）**：
- Runs 页：自动评估、策略编辑（全局/项目）、评估结果阅读与快捷跳转
- Learning Artifacts：列表过滤、URL 固定、Artifact 详情摘要与 Issues 专用视图
...
| **Layer 2 - 平台服务层** | REST API 端点（262个，含 FDE 6模块） | ✅ 已实现 |
| **Layer 3 - 应用接入层** | 部分实现（channels/cli/workbench/events） | ⚠️ 部分实现 |
...
| 模块 | 页面 | 实现状态 |
|------|------|---------|
| **执行引擎管理** | 执行引擎状态 | ✅ 已实现 (Diagnostics/Runs) |
| | 执行配置管理 | ✅ 已实现 (Core/Plugins) |
| | 执行日志查询 | ✅ 已实现 (Diagnostics/Traces) |
| **智能体管理** | Agent列表 | ✅ 已实现 (Core/Agents + Workspace/Agents) |
| | Agent详情 | ✅ 已实现 |
| | 创建Agent | ✅ 已实现 |
| **技能管理** | Skill列表 | ✅ 已实现 (Core/Skills + Workspace/Skills) |
| | Skill详情 | ✅ 已实现 (Core/SkillPacks) |
| | 注册Skill | ✅ 已实现 |
| **记忆系统管理** | 会话列表 | ✅ 已实现 (Core/Memory) |
| | 记忆统计 | ✅ 已实现 |
| | 会话详情 | ✅ 已实现 |
| **知识库管理** | 知识库列表 | ✅ 已实现 (Workspace/Wiki) |
| | 文档管理 | ✅ 已实现 |
| | 检索测试 | 🔜 待实现 |
| **适配器管理** | 适配器列表 | 🔜 待实现 |
| | 适配器配置 | 🔜 待实现 |
