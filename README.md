...
│                  aiPlat-app (Layer 3)                      │
│              应用层 - 面向用户的应用                          │
│         Gateway, CLI, Workbench, Services                  │
└────────────────────┬───────────────────────────────────────┘
                     │ 依赖
                     ↓
┌─────────────────────────────────────────────────────────────┐
...
│         平台服务层 - API、认证、多租户                        │
│   API, Auth, Tenants, Gateway, Messaging, Registry        │
└────────────────────┬───────────────────────────────────────┘
                     │ 依赖
                     ↓
┌─────────────────────────────────────────────────────────────┐
...
基础设施层提供数据库、LLM 客户端、向量数据库等基础服务。核心层提供 Agent、Skill、编排等 AI 能力。平台层提供 API、认证、多租户等平台服务。应用层提供 Gateway、CLI、Web UI 等用户界面。

### 运行服务

启动 API 服务、Gateway 或使用 CLI 工具进行管理。
