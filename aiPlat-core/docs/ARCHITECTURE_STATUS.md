...
| 模块 | 说明 | 代码位置 | 文档位置 |
|------|------|---------|---------|
| `kernel/` | 核心执行契约（ExecutionRequest/ExecutionResult） | `core/harness/kernel/types.py` | `docs/design/kernel_orchestrator/` |
| `syscalls/` | 系统调用层（LLM/Tool/Skill 调用） | `core/harness/syscalls/` | `docs/design/kernel_orchestrator/02-syscalls-and-gates.md` |
| `engines/` | 执行引擎抽象 | `core/harness/execution/engines/` | `docs/design/kernel_orchestrator/03-engines-and-routing.md` |
| `gates/` | 门控系统（Context/Policy/Resilience/Trace） | `core/harness/infrastructure/gates/` | `docs/design/kernel_orchestrator/02-syscalls-and-gates.md` |
| `orchestration/` | 编排器（生成执行计划） | `core/orchestration/orchestrator.py` | 规划中 |
| `learning/` | 学习工件系统（版本化、可追溯） | `core/learning/` | 规划中 |
| `assembly/` | 组装器（Context/Prompt） | `core/harness/assembly/` | 规划中 |

---

### Phase 10 新增模块（评估/证据/回归/策略）

| 模块 | 说明 | 代码位置 | 文档位置 |
|------|------|---------|---------|
| `harness/evaluation/` | 自动评估与回归门控（evidence_pack/diff、tag_assertions、policy） | `core/harness/evaluation/` | `docs/design/evaluation/auto-eval-and-regression.md` |
| `learning_artifacts` 扩展 | 新增 artifacts kind（evidence_pack/evidence_diff/evaluation_policy） | `core/learning/types.py` | 同上 |
| project policy | 项目级策略与继承覆盖（project ⊕ system/default） | `core/server.py` | 同上 |

## 设计文档保真度

| 文档 | 保真度 | 主要问题 |
|------|--------|---------|
| `skills/architecture.md` | ✅ 中 | 发现→注册、Manager↔Registry 桥接、SkillContext.tools 注入、Fork 模式、版本管理均已实现；Skill 进化仍为仅文档 |
...
| `harness/index.md` | ✅ 中 | Phase 9 新增模块已记录（kernel/syscalls/engines/gates/orchestration/learning/assembly）；所有横幅已更新 |
...
| `design/kernel_orchestrator/` | ✅ 高 | Phase 9 新增架构设计文档（6篇：架构/契约/系统调用/引擎/安全/迁移） |
| `design/evaluation/auto-eval-and-regression.md` | ✅ 高 | 新增 As-Is 文档，覆盖 auto-eval/证据/回归/策略/项目级继承，并给出证据索引 |
