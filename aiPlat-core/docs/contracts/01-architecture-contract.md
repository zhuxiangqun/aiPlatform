...
- **provider YAML 驱动补完（P2-A3）**：`ModelManager._API_PROVIDERS` 硬编码集合改为 `_api_provider_ids()`（从 `config/providers.yaml` 按 `type=external` 派生，5min 缓存，YAML 缺失回退硬编码）；新增 external provider 零代码。**2026-08-24 生态广度一批（6→14）**：+qwen/groq/mistral/cohere/cerebras/together/xai/novita（8 家族）。**2026-08-25 生态广度二批（14→22）**：+siliconflow/moonshot/minimax/zhipu/baichuan/stepfun/deepinfra/fireworks（8 家族）；**三批（22→30）**：+gemini/nvidia/huggingface/upstage/arcee/zai/xiaomi/nous（8 家族）——provider 总数 22→30（外部 API 29 + ollama local），向 Hermes 38 方向继续收窄。
...
- **消息渠道适配器（P1-A4, 2026-08-18）**：`get_channel_adapter(name)` 统一解析（7 渠道：3 内置 telegram/slack/webchat + 4 扩展 discord/wecom/email/dingtalk，`wecom`→`wechat` 别名）。扩展适配器在 `aiPlat-app/channels/adapters/` 注册进 `ChannelDispatcher`（merged）。platform `POST /platform/channels/{id}/test` 校验适配器存在（fail-loud 422）。registry 补登 `get_channel_adapter`。**2026-08-23 渠道广度延伸（7→10）**：+whatsapp/lark/teams 适配器（WhatsApp Cloud API / 飞书机器人 / Teams Bot Framework），`ADAPTERS` 注册 + `ChannelType` 补枚举；`get_channel_adapter`/`POST /channels/{id}/test` 动态解析自动覆盖，零 platform 改动（对齐 Hermes 22 平台广度方向）。**2026-08-24 渠道广度延伸二批（10→14）**：+signal/matrix/mattermost/line 适配器（signal-cli json / Matrix client-server API / Mattermost webhook / LINE Messaging API），`ADAPTERS` 注册 + `ChannelType` 补 4 枚举——渠道 10→14（3 内置 + 11 扩展），继续对齐 Hermes 22 平台广度。**三批（14→18）**：+qq/reddit/github/sms 适配器（QQ 机器人 / Reddit comment / GitHub webhook / Twilio-style SMS），`ADAPTERS` 注册 + `ChannelType` 补 4 枚举——渠道 14→18（3 内置 + 15 扩展）。**四批收官（18→22，2026-08-25）**：+google_chat/homeassistant/irc/ntfy 适配器（Google Chat event / Home Assistant 事件 / IRC PRIVMSG / ntfy.sh publish），`ADAPTERS` 注册 + `ChannelType` 补 4 枚举——渠道 18→22（3 内置 + 19 扩展），**对齐 Hermes 22 平台广度收官**。
...
- **事件源纯度——Fork 会话 + 血缘（2026-08-24）**：`PipelineRunStore.fork_run_from_events(source_run_id, new_run_id, project_id, *, note)` 从源 run 事件日志折叠派生状态 → 新 run 行继承分叉点进度/通过率并从 executing 继续；`pipeline_forked` 事件（append-only，含 parent_run_id + note + source_event_count + 继承 current_stage_idx/pass_rate）写入子 run 事件日志，`replay_run_events` 折叠该事件使**子 run 状态可纯从自身事件重建**；`list_forked_runs(parent_run_id, limit)` 血缘查询。接线：`POST /pipeline/pipelines/runs/{run_id}/fork` + `GET /pipeline/pipelines/runs/{run_id}/forks`（core api router，经 CoreFacade 取 store）。契约：run_id 语义扩展为可派生（fork 生成新 run_id，父 id 记录于子事件）；事件日志保持 append-only（fork 不写回父）；state+event 双写与 syscall 三通道唯一性均不变。

- **continuable 子代理编排（2026-08-24）**：`coordinator.continue_execution(instance_id, message)`（复用 `execute_single` 创建的 agent——`SubagentInstance.agent_ref` 保留，向同一会话追加消息重执行；conversational agent `_conversation_history` 天然多轮）+ `execute_single` 返回真实 instance key（`SubagentResult.instance_id` = `{session_id}:{name}`）+ `InProcessProvider.continuation` 委托 coordinator（`capabilities.continuation=True` 名实相符）+ `coordinator.send_message` → in_process 续接成功。契约：continuable 是**编排层能力**（DSH send_message 对齐）——agent 上下文隔离在会话内（`_conversation_history` 不跨 instance）；fail-loud（未知 instance / 无 agent_ref → 明确错误，不静默假成功）；多轮续接由 conversational agent 天然支持，无需新执行引擎。
...
- **治理/架构路线图 REAL 项修复（2026-08-25）**：code_graph 增量同步补新文件发现（未索引 .py 浅层入图，≤100 上限）；`scripts/coupling_metrics.py` 耦合度量基线（AST import-degree + baseline ratchet）；harness 48 子目录 BOUNDARY.yaml 全量补齐 + 架构守卫 §94b 检查；记忆测试导入路径迁移 `harness.*`→`core.harness.*`；pre-commit 框架决策文档化（手工 hook 为实际生效实现，config 保留声明式参考）。契约：耦合 ratchet 只允许下降（新增模块不得抬高 avg_degree/max_degree）；BOUNDARY 声明为 harness 子目录强制项（缺则守卫 FAIL）。

- **意图路由统一检索（2026-08-28，AnySearch 借鉴 P0-2）**：新增 `sys_routed_retrieve`（`core/harness/syscalls/retrieval.py`）——查询理解驱动路由（`_route_intent` 轻量意图判定：code/knowledge/web 三通道，T1 关键词无 LLM），路由到匹配通道（code → `sys_code_search`；knowledge → `sys_knowledge_retrieve` RRF 融合；web → urllib 直调 DuckDuckGo JSON Instant Answer，**不 import apps 层**，保持 harness 单向依赖）；通道不可用降级（code→knowledge→web）；web 可关（`include_web=False`）；返回统一 `{text, score, source}` 事实条目 + 信源标注 `sources[]`。接线：`RoutedRetrieveTool`（`core/apps/tools/routed_retrieve.py`）经 server.py 工具注册表供 Agent `sys_tool_call` 调用。契约：检索类 syscall 不属三通道唯一性破坏（非 LLM/tool/skill 三通道）；harness 层禁止 import apps 层（§5.14 单向依赖）——web 检索在 harness 层以 urllib 直调实现。
