"""
Builder team service — team CRUD + file persistence + execution via PipelineEngine.
"""
...
class BuilderTeamService:
    ...
    async def run_team(self, team_id: str, description: str = "",
        ...
        self._engines[team_id] = engine

        state = await engine.initialize(team_id, description, prd_data)
    ...
    async def approve_stage(self, team_id: str) -> Dict[str, Any]:
        engine = self._engines.get(team_id)
        if not engine:
