...
# Default pricing per 1M tokens (USD), overridable via env
_DEFAULT_PRICING = {
    "deepseek-v4-pro":     {"input": 0.55, "output": 2.19},
    "deepseek-chat":       {"input": 0.14, "output": 0.28},
    "deepseek-coder":      {"input": 0.14, "output": 0.28},
    "qwen2.5-coder:7b":    {"input": 0.0,  "output": 0.0},   # local
    "qwen2.5:3b":          {"input": 0.0,  "output": 0.0},   # local
...
class CostTracker:
    ...
    def _load_pricing_env(self):
        try:
            import json
            env_data = os.getenv("AIPLAT_MODEL_PRICING", "")
            if env_data:
