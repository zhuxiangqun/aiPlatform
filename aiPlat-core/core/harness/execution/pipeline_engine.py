...
class PipelineEngine:
    ...
    def _load_eval_model(self) -> Any:
        eval_model_name = os.getenv("AIPLAT_EVAL_MODEL", "").strip()
        if eval_model_name:
    ...
    def _try_constraint_action(self, stage: PipelineStageConfig, state: Dict) -> bool:
        ...
        if action == "switch_fallback_model":
            fb_model = os.getenv("AIPLAT_FALLBACK_MODEL", os.getenv("AIPLAT_LLM_MODEL", "deepseek-chat"))
            try:
    ...
    def _load_default_model(model_name: str = "", *, category: str = "default") -> Any:
        ...
        if not model_name:
            if category == "agent":
                model_name = os.getenv("AIPLAT_AGENT_MODEL", "deepseek-reasoner")
            else:
                model_name = os.getenv("AIPLAT_LLM_MODEL", "deepseek-chat")
    ...
    async def _run_stage_core(
        ...
        if node_type == 'llm':
            ...
            llm_model_name = node_cfg.get('model') or (os.getenv("AIPLAT_LLM_MODEL", "deepseek-chat"))
        ...
        if node_type == 'knowledge':
            ...
            if kb_vector_provider == 'lancedb':
                try:
                    ...
                    pass  # lancedb not installed, fall through to kb_query
                except Exception as e:
...
Output format: JSON array of {{"rank": 1, "score": 0.95, "content": "..."}}"""
                        ...
                        rerank_resp = await sys_llm_generate(
                            os.getenv("AIPLAT_LLM_MODEL", "deepseek-chat"),
        ...
        if node_type == 'tool':
            ...
            try:
                ...
                from core.harness.syscalls.tools import sys_tool_call
        ...
        async with self._model_lock:
            ...
            if not getattr(stage, 'model', None):
                ...
                if not has_errors and is_short and is_first_run:
                    import os
                    simple_model = os.getenv("AIPLAT_SIMPLE_TASK_MODEL") or os.getenv("AIPLAT_LLM_MODEL", "deepseek-chat")
