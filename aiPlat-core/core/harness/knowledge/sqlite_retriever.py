    ...
    def _load_embeddings(self, conn: sqlite3.Connection, query: str, limit: int) -> list:
        ...
        try:
            ...
            sql = f"""
                ...
                SELECT e.element_id, e.doc_id, e.type, e.page_idx, e.text, e.cells_json, e.meta_json, emb.vector_json
    ...
    def _row_to_entry(self, row: dict) -> KnowledgeEntry:
        ...
        if cells_raw:
            ...
            try:
                ...
                cells = _json.loads(cells_raw) if isinstance(cells_raw, str) else cells_raw

                if isinstance(cells, list) and cells:

                    metadata.tags.append("has_table")

                    if not content:

                        from core.harness.document.converters._mineru import _cells_to_markdown

                        content = _cells_to_markdown(cells)

            except Exception:

                logging.getLogger(__name__).debug('_row_to_entry failed', exc_info=True)
    ...
    *,
    ...
    if hyde_enabled:
        ...
        try:
            ...
            hyde_model = create_selected_adapter(best_model_for_purpose("doc_llm"))

        except Exception:

            logging.getLogger(__name__).debug('create_sqlite_retriever failed', exc_info=True)
    ...
    return KnowledgeRetriever(
        ...
        hyde_enabled=hyde_enabled and hyde_model is not None,
