...
class PolicyGate:
    ...
    def check_route_access(path: str, role: str, method: str = "GET") -> bool:
        ...
        from core.api.core_facade import get_route_permissions, get_method_restrictions

        ROUTE_PERMISSIONS = get_route_permissions()

        METHOD_RESTRICTIONS = get_method_restrictions()
    ...
    async def check_tool(self, *, user_id: str, tool_name: str, tool_args: Optional[Dict[str, Any]] = None) -> PolicyResult:
        ...
        if not perm_mgr.check_permission(user_id, tool_name, Permission.EXECUTE):
            ...
            # PR #3: 工具调用被拒绝 — 归因到 D2_tools

            try:

                from core.harness.meta.profile_registry import set_failure_domain

                set_failure_domain("D2_tools")

            except Exception:

                logging.getLogger(__name__).debug('check_tool failed', exc_info=True)
            return PolicyResult(
    ...
    # ══════════════════════════════════════════════════════════════
    # Phase 42: check_tool_3d — 三维权限融合 (Role × Purpose × Marking)
    # ══════════════════════════════════════════════════════════════

    async def check_tool_3d(
        self,
        *,
        user_id: str,
        tool_name: str,
        ...
        role: str = "",
        marking_level: int = 1,              # PUBLIC=1 INTERNAL=2 CONFIDENTIAL=3 RESTRICTED=4
        trace_context: Optional[Dict[str, Any]] = None,
    ) -> PolicyResult:
        """三维权限融合检查: Role ∩ Purpose ∩ Marking.

        先跑标准 check_tool (Role-based), 再叠加 Purpose + Marking 限制。
        任一维度 DENY → 整体 DENY.

        设计原则 (Palantir 对齐):
          工具调用权限 = Role能做的 ∩ Purpose允许的 ∩ Marking可见的 → 运行时动态计算
        """
        # Dimension 1: Standard RBAC (Role) check
        result = await self.check_tool(
            user_id=user_id, tool_name=tool_name, tool_args=tool_args
        )
        if result.decision != PolicyDecision.ALLOW:
            return result

        # Dimension 2: Purpose constraint
        try:
            ...
            if not purpose_check["allowed"]:
                return PolicyResult(
                    decision=PolicyDecision.DENY,
                    reason=f"Layer-Purpose: {purpose_check['reason']}",
                    scope=getattr(result, "scope", PolicyScope.SESSION),
                )

            if purpose_check.get("require_approval"):
                return PolicyResult(
                    decision=PolicyDecision.APPROVAL_REQUIRED,
                    reason=f"Layer-Purpose: approval required for purpose '{purpose_id}'",
                    scope=getattr(result, "scope", PolicyScope.SESSION),
                )
        ...
        # Dimension 3: Marking clearance on tool args
        if trace_context:
            try:
                from core.harness.infrastructure.gates.marking_propagation import check_tool_args_markings

                marking_max = trace_context.get("_max_marking_level", marking_level)
                allowed, reason = check_tool_args_markings(
                    tool_args, max_allowed_level=marking_max,
                    collection_id=trace_context.get("collection_id", "default"),
                )
                if not allowed:
                    return PolicyResult(
                        decision=PolicyDecision.DENY,
