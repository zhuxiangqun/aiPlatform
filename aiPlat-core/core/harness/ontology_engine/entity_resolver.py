...
class EntityResolver:
    ...
    def build_alias_index(self, instances: List[Dict[str, Any]],
        ...
        Also expands using synonym map for cross-name matching.
        ...
        for inst in instances:
            ...
            eid = inst.get("entity_id", canonical)

            index[canonical] = eid

            # P2-A: also index synonyms pointing to this canonical
            ...
            if synonym_map:
                ...
                for syn, can in synonym_map.items():
                    ...
                    if can.lower() == canonical.lower():
                        ...
                        index[self.normalize_term(syn, domain_id=domain_id)] = eid
    ...
    def _score_pair(
        ...
        # P2-A: L0 synonym match — if names are known synonyms, score 0.9+

        syn_score = self._score_pair_synonym(name_a, name_b)
    ...
    # ── P2-A: Synonym support ─────────────────────────────────────



    _synonym_map_cache: Optional[Dict[str, str]] = None



    def _load_synonym_map(self) -> Dict[str, str]:

        """Load synonym → canonical map from YAML ontologies + synonyms.yaml.



        Lazy load + cache. All imports inside function body to avoid circular deps.
        ...
        try:
            ...
            from core.harness.knowledge.knowledge_ontology import load_synonyms

            loaded = load_synonyms()
            ...
            logging.getLogger(__name__).debug('_load_synonym_map failed', exc_info=True)
        try:
            ...
            for did in DomainRouter.list_domains():

                try:

                    ontology = load_ontology_from_yaml(

                        f"~/.aiplat/ontologies/{did}.yaml")

                    for cls in ontology.classes:

                        label = (getattr(cls, 'label', '') or '').lower()

                        for syn in (getattr(cls, 'synonyms', []) or []):

                            if syn and syn.lower() != label:
            ...
            logging.getLogger(__name__).debug('_load_synonym_map failed', exc_info=True)
        self._synonym_map_cache = sm

        return sm



    def _score_pair_synonym(self, name_a: str, name_b: str) -> float:

        """Return 0.9+ if names are known synonyms, 0.0 otherwise."""

        na = self.normalize(name_a).lower()

        nb = self.normalize(name_b).lower()

        if na == nb:

            return 0.0

        sm = self._load_synonym_map()
        ...
        if sm.get(na) == nb or sm.get(nb) == na:
