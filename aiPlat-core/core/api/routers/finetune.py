...
async def start_training(body: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    try:
        from core.api.core_facade import get_rl_trainer  # P0-A2: 经 CoreFacade
...
async def start_distillation(body: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    try:
        from core.api.core_facade import get_distillation_engine  # P0-A2: 经 CoreFacade
...
async def start_scratch_training(body: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    try:
        from core.api.core_facade import get_full_training_engine, FullTrainingConfig  # P0-A2: 经 CoreFacade
...
async def get_scratch_status(job_id: str) -> ScratchJobResult:
    ...
    try:
        from core.api.core_facade import get_full_training_engine  # P0-A2: 经 CoreFacade
        engine = get_full_training_engine()
        status = engine.get_status(job_id)
        if not status:
            raise HTTPException(status_code=404, detail="Job not found")
