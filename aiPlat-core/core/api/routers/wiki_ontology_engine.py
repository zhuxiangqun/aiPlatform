...
async def _get_kb_docs_for_domain(domain_id: str, limit: int) -> list:

    """Find KB markdown docs for engine processing (videos too slow for real-time)."""

    import sqlite3, os

    from pathlib import Path

    home = Path(os.getenv("AIPLAT_HOME", Path("~").expanduser() / ".aiplat"))

    kb_db = str(home / "kb" / "tenants" / "default" / "kb.sqlite3")

    if not Path(kb_db).exists():

        return []

    from core.harness.infrastructure.infra_bridge import create_infra_database_client

    conn = create_infra_database_client(kb_db)
    ...
    try:
        ...
        cur.execute(
            ...
            "SELECT doc_id, source_uri, kind FROM documents WHERE status='ready' AND kind='markdown' LIMIT ?",
        ...
        logging.getLogger(__name__).debug('_get_kb_docs_for_domain failed', exc_info=True)
    finally:

        conn.close()

    return [{"doc_id": r[0], "file_path": r[1], "kind": r[2]} for r in rows]





async def _process_kb_doc_to_wiki(engine, doc: dict, domain_id: str, collection_id: str) -> list:

    """Process a markdown KB doc through the ontology engine pipeline."""
    ...
    if not file_path or not os.path.isfile(file_path):
        ...
        return [{"doc_id": doc_id, "status": "file_not_found"}]
    ...
    try:
        ...
        result = await engine.process_chunks([{"text": text, "source": file_path}])

        instances = getattr(result, "instances", None) or []
        ...
        for inst in instances:
            ...
            fm = inst.get("frontmatter", {}) if isinstance(inst, dict) else {}
            ...
            try:
                ...
                cat = fm.get("category", "entities") if isinstance(fm, dict) else "entities"

                tags = list(fm.get("tags", []) or []) if isinstance(fm, dict) else []

                write_page(title=title, body=str(body)[:8000], category=str(cat),

                           collection_id=collection_id, tags=tags)

                written += 1

            except Exception as e:

                logging.warning(f"Wiki write failed for {title}: {e}")

        return [{"doc_id": doc_id, "status": "processed", "instances": written}]

    except Exception as e:

        logging.warning(f"Pipeline failed for {doc_id}: {e}", exc_info=True)
...
async def build_instances_batch(domain_id: str, collection: str = "", limit: int = 50):
    ...
    if not pages:
        ...
        # ── v2.8: try KB markdown docs when no wiki pages exist ──

        kb_docs = await _get_kb_docs_for_domain(domain_id, limit)

        if kb_docs:

            results = {"domain_id": domain_id, "collection": cid,

                       "processed": 0, "from_kb": True, "details": []}

            import asyncio

            tasks = [_process_kb_doc_to_wiki(engine, doc, domain_id, cid) for doc in kb_docs]
            ...
            for br in batch_results:
                ...
                if isinstance(br, Exception):
                    ...
                    results["details"].append({"error": str(br)})

                else:

                    results["processed"] += 1

                    results["details"].extend(br)
...
@router.post("/domains/{domain_id}/sync-search-index", response_model=Dict[str, Any])
async def sync_search_index(domain_id: str):
    """Sync graph entity classes to Wiki FTS5 + vector cache for search discoverability.

    Zero LLM calls. Reads GraphIndex entities, creates/updates entity class index
    ...
    try:
        ...
        if node_count == 0:
            return {"domain_id": domain_id, "synced": 0, "classes": 0, "nodes": 0,
                    "message": "No graph entities found for this domain"}
        
        synced = graph.sync_to_wiki()
        
        # Rebuild FTS5 to pick up new pages
        try:
        ...
        return {
            ...
            "classes": class_count,
            "nodes": node_count,
            "message": f"Synced {synced} entity class index pages to Wiki/FTS5"
        }
    except Exception as e:
...
@router.post("/domains/sync-search-index-all", response_model=Dict[str, Any])
async def sync_search_index_all():
    """Sync all domain graph entity classes to Wiki FTS5 + vector cache.

    Iterates all graph database files, syncs entities to Wiki for each domain.
    Zero LLM calls. Returns per-domain sync counts.
    """
    import os as _os, glob as _glob
    graph_dir = _os.path.expanduser(
        _os.getenv("AIPLAT_HOME", _os.path.expanduser("~/.aiplat"))) + "/graph"
    ...
    if _os.path.isdir(graph_dir):
        for db_file in sorted(_glob.glob(_os.path.join(graph_dir, "*.db"))):
            ...
            try:
                ...
                results[domain_id] = {
                    "synced": synced,
                    "classes": len(graph.get_entity_classes()),
                    "nodes": node_count
                }
                total_synced += synced
                total_classes += len(graph.get_entity_classes())
                total_nodes += node_count
            except Exception as e:
                results[domain_id] = {"error": str(e)[:200]}
