...
async def delete_page(title: str, collection: str = "default"):
    from core.api.core_facade import delete_page as _del
...
def _build_diagnosis(run_id: str, state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from core.harness.execution.failure_clusterer import classify_verifier_cause  # P0-A2 修复: 恢复原模块(定义处)
    from core.harness.execution.failure_clusterer import classify_causal_status  # P0-A2 修复: 恢复原模块(定义处)
    from core.harness.execution.failure_clusterer import classify_abstract_mechanism  # P0-A2 修复: 恢复原模块(定义处)
...
async def get_wiki_schema(collection: str = "default", domain: str = ""):
    ...
    try:
        from core.harness.knowledge.knowledge_ontology import OBJECT_PROPERTIES  # P0-A2 修复: 恢复原模块(定义处)
        from core.harness.knowledge.knowledge_ontology import get_classes_with_templates
        from core.harness.knowledge.knowledge_ontology import get_extended_class  # P0-A2 修复: 恢复原模块(定义处)
        from core.harness.knowledge.knowledge_ontology import load_collection_extension  # P0-A2 修复: 恢复原模块(定义处)
        from core.policy.object_permission import AI  # P0-A2 修复: 恢复原模块(定义处)
...
async def register_inference_rule(body: Dict[str, Any]):
    ...
    try:
        from core.harness.knowledge.knowledge_validator import InferenceRule  # P0-A2 修复: 恢复原模块(定义处)
        from core.harness.knowledge.knowledge_validator import RuleTrigger  # P0-A2 修复: 恢复原模块(定义处)
        from core.harness.knowledge.knowledge_validator import register_rule  # P0-A2 修复: 恢复原模块(定义处)
...
async def batch_atomize_endpoint(limit: int = 10, category: str = "topics", collection: str = "default"):
    ...
    try:
        ...
        from core.harness.knowledge.wiki_engine import write_atom  # P0-A2 修复: 恢复原模块(定义处)
        from core.harness.knowledge.wiki_engine import _extract_sub_concepts  # P0-A2 修复: 恢复原模块(定义处)
