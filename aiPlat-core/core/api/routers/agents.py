...
async def resolve_pipeline_hitl(pipeline_id: str, request: dict):
    ...
    Body: {"action": "approve"|"reject"|"grilling", "feedback": "optional comment"}

    

    v2.9: Added grilling action — starts a GrillingBridge interview for structured

    requirements review instead of a binary approve/reject.
    ...
    try:
        ...
        if action == "grilling":
            ...
            domain_id = engine._state.get("_hitl_domain_id", "") or engine._state.get("domain_id", "ai-knowledge")

            from core.api.core_facade import start_grilling

            r = start_grilling("pipeline_hitl", domain_id, {"task": feedback})

            return {"status": "grilling_started", "pipeline_id": pipeline_id, "session": r}
...
@router.post("/agents/{agent_id}/diff", response_model=Dict[str, Any])

async def agent_config_diff(agent_id: str, req: AgentDiffRequest):

    """Compute structured diff between two versions of an agent's AGENT.md frontmatter.
    ...
    return {"agent_id": agent_id, **compute_agent_diff(req.old_content, req.new_content)}



# ── v2.9: Skill Export API ──



@router.get("/skills/{skill_name}/export", response_model=Dict[str, Any])

async def export_skill(skill_name: str, format: str = "all"):

    """Export a skill definition to external framework specs (OpenAI/LangChain/Anthropic)."""
    ...
    skill_path = Path(os.getenv("AIPLAT_HOME", Path("~").expanduser() / ".aiplat")) / "skills" / skill_name / "SKILL.md"

    if not skill_path.exists():

        # Try engine skills

        skill_path = Path(__file__).resolve().parent.parent.parent.parent / "engine" / "skills" / skill_name / "SKILL.md"
    ...
    return {"skill_name": skill_name, "format": format, **exporter.export_all(content, format)}



# ── v2.9: Business Value Report API ──



@router.get("/business-value/{project_id}", response_model=Dict[str, Any])

async def get_business_value(project_id: str, project_name: str = "", monthly_count: int = 100):

    """Generate a project-level business value report for renewal decisions."""

    from core.harness.evaluation.business_value import generate_renewal_report

    return generate_renewal_report(project_id, project_name or project_id, monthly_count)





@router.get("/business-value/{project_id}/renewal", response_model=Dict[str, Any])

async def get_renewal_report(project_id: str, project_name: str = "", monthly_count: int = 100):

    """Generate a detailed renewal report with business KPIs and recommendations."""

    from core.harness.evaluation.business_value import generate_renewal_report

    report = generate_renewal_report(project_id, project_name or project_id, monthly_count)

    # Add markdown export support

    report["markdown"] = _build_renewal_markdown(report)
...
def _build_renewal_markdown(report: Dict[str, Any]) -> str:

    """Build a Markdown renewal report."""

    lines = [

        f"# {report['project_name']} · 业务价值报告",

        f"",

        f"**评级**: {report['grade']} ({report['score']}分)",

        f"**月执行量**: {report['monthly_exec_count']} 次 · 节省 {report['hours_saved']} 小时",
    ...
    for k in report["kpis"]:
        ...
        lines.append(f"- **{k['label']}**: {k['value']} {k['trend']} — {k['detail']}")
