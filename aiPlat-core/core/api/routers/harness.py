...
def _mgr(rt: Optional[KernelRuntime]):
...
async def list_hooks(rt: RuntimeDep = None):
    ...
    return {
        "hooks": [{"id": h.id, "name": h.name, "type": h.type, "priority": h.priority, "enabled": h.enabled} for h in hooks]
    }
...
async def list_coordinators(rt: RuntimeDep = None):
    ...
    return {
        "coordinators": [
            {"id": c.id, "pattern": c.pattern, "agents": c.agents, "status": c.status, "created_at": c.created_at.isoformat() if c.created_at else None}
