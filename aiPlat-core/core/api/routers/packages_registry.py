...
def _pkg_managers():
    ...
    try:
        ...
        logging.getLogger(__name__).debug('_pkg_managers failed', exc_info=True)
    return None, None
...
async def _require_package_approval(operation: str, user_id: str, details: str, metadata: dict):
    ...
    try:
        ...
        logging.getLogger(__name__).debug('_require_package_approval failed', exc_info=True)
    return None
...
def _engine_mcp_manager():
    ...
    try:
        ...
        logging.getLogger(__name__).debug('_engine_mcp_manager failed', exc_info=True)
    return None
