...
def test_approval_layer_policy_skill_only(tmp_path, monkeypatch):
    out_dir = _setup_common_env(tmp_path, monkeypatch)
    monkeypatch.setenv("AIPLAT_APPROVAL_LAYER_POLICY", "skill_only")
    ...
    with TestClient(server.app) as client:
        ...
        # No second pending approval (tool-level should be bypassed by reusing skill approval).
        pending = client.get("/api/core/approvals", params={"run_id": run_id, "status": "pending", "limit": 50, "offset": 0})
        assert pending.status_code == 200, pending.text
        assert int(pending.json().get("total") or 0) == 0


@pytest.mark.integration
def test_approval_layer_policy_tool_only(tmp_path, monkeypatch):
    out_dir = _setup_common_env(tmp_path, monkeypatch)
    monkeypatch.setenv("AIPLAT_APPROVAL_LAYER_POLICY", "tool_only")
    ...
    with TestClient(server.app) as client:
        ...
        # In tool_only, skill approval is bypassed; tool approval should appear.
        w1 = client.post(
            f"/api/core/runs/{run_id}/wait",
            headers={"X-AIPLAT-TENANT-ID": "t_demo", "X-AIPLAT-ACTOR-ID": "admin", "X-AIPLAT-ACTOR-ROLE": "admin"},
