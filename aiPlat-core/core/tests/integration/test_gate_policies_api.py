...
def test_gate_policy_crud(tmp_path, monkeypatch, mount_platform_governance):
    ...
    mount_platform_governance(server.app)

    hdr = {"X-AIPLAT-TENANT-ID": "t_demo", "X-AIPLAT-ACTOR-ID": "admin", "X-AIPLAT-ACTOR-ROLE": "admin"}
    with TestClient(server.app) as client:
        r0 = client.get("/api/platform/governance/gate-policies", headers=hdr)
        assert r0.status_code == 200

        boot = client.post("/api/platform/governance/gate-policies/bootstrap?force=true", headers=hdr, json={})
        assert boot.status_code == 200, boot.text
        assert boot.json().get("seeded") is True

        r1 = client.get("/api/platform/governance/gate-policies", headers=hdr)
        ...
        up = client.put(
            "/api/platform/governance/gate-policies/prod",
            headers=hdr,
        ...
        g1 = client.get("/api/platform/governance/gate-policies/prod", headers=hdr)
        ...
        vers = client.get("/api/platform/governance/gate-policies/prod/versions", headers=hdr)
        ...
        rb = client.post("/api/platform/governance/gate-policies/prod/rollback", headers=hdr, json={"version": v0})
        assert rb.status_code == 200, rb.text
        assert rb.json().get("change_id")

        sd = client.post("/api/platform/governance/gate-policies/prod/set-default", headers=hdr)
        assert sd.status_code == 200
        assert sd.json().get("default_id") == "prod"
        assert sd.json().get("change_id")

        d0 = client.delete("/api/platform/governance/gate-policies/prod", headers=hdr)
        assert d0.status_code == 200
        assert d0.json().get("change_id")


@pytest.mark.integration
def test_gate_policy_change_control_workflow(tmp_path, monkeypatch, mount_platform_governance):
    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("AIPLAT_EXECUTION_DB_PATH", str(tmp_path / "exec.sqlite3"))
    monkeypatch.setenv("AIPLAT_RBAC_MODE", "disabled")
    ...
    hdr = {"X-AIPLAT-TENANT-ID": "t_demo", "X-AIPLAT-ACTOR-ID": "admin", "X-AIPLAT-ACTOR-ROLE": "admin"}
    with TestClient(server.app) as client:
        client.post("/api/platform/governance/gate-policies/bootstrap?force=true", headers=hdr, json={})

        prop = client.post(
            "/api/platform/governance/gate-policies/prod/propose",
            headers=hdr,
        ...
        ap0 = client.post(f"/api/platform/governance/gate-policies/changes/{change_id}/apply", headers=hdr, json={})
        ...
        ap1 = client.post(f"/api/platform/governance/gate-policies/changes/{change_id}/apply", headers=hdr, json={})
