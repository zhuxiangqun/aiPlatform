...
def test_phase624_configurable_evidence_cap_marks_truncation(tmp_path):
    """
    Phase 6.24 acceptance:
    - --max-linked-evidence controls evidence cap
    - output and regression_report evidence contain truncation flags
    """
    ...
    assert out.get("linked_evidence_cap") == 20
    assert out.get("linked_current_truncated") is True

    rep = anyio.run(store.get_learning_artifact, report_id)
    evidence = rep["payload"]["deltas"]["evidence"]
    assert evidence["linked_evidence_cap"] == 20
    assert evidence["linked_current_truncated"] is True
    assert len(evidence["linked_current_execution_ids"]) == 20
