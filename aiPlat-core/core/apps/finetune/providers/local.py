...
class LocalFineTuneProvider:
    ...
    def available(self) -> bool:
        ...
        try:
            ...
            pass  # psutil not installed, assume OK  # noqa: optional-dependency



        # 3. llama.cpp tools (for final GGUF export)

        from core.apps.finetune.gguf_exporter import GGUFExporter
    ...
    def create_job(self, file_id: str, model: str, suffix: str,
        ...
        from core.apps.finetune.mlx_trainer import MLXLoRATrainer
        ...
        if trainer and trainer.is_running:
            ...
            if not job.get("_gguf_exported"):
                ...
                try:
                    ...
                    logging.getLogger(__name__).debug('get_job failed', exc_info=True)
