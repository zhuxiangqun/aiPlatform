...
class FDEBuilderOrchestrator:
    ...
    async def clarify(self, requirement: str, fde_context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            from core.api.core_facade import fde_clarify



            history = (fde_context or {}).get("history", [])

            result = await fde_clarify(
    ...
    async def build_and_deploy(self) -> Dict[str, Any]:
        ...
        try:
            ...
            from core.api.core_facade import get_builder_project_service
            ...
            # ── Feed knowledge gaps to CandidatePool for cross-session dedup ──

            try:

                from core.harness.knowledge.candidate_pool import CandidatePool

                pool = CandidatePool(self.session.domain_id)

                gaps = getattr(self.session, 'knowledge_gaps', []) or []
                ...
                for g in gaps:
                    ...
                    pool.add_gap(
                        ...
                        gap_type=g.get('type', 'missing_class'),
                ...
                pool.check_triggers()  # may fire ActiveSynthesis if >=3 sessions confirm

            except Exception:

                logging.getLogger(__name__).debug('build_and_deploy failed', exc_info=True)
