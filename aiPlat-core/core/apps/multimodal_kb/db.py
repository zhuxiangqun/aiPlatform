...
class KBSqlite:
        ...
        with self.connect() as conn:
            conn.execute(
                ...
                INSERT OR REPLACE INTO kb_elements(
                  tenant_id, element_id, doc_id, type, page_idx, bbox_json, text, cells_json, asset_id, meta_json, created_at
                )
                VALUES(?,?,?,?,?,?,?,?,?,?,strftime('%s','now'))
                """,
                (
    ...
    def delete_doc_data(self, *, tenant_id: str, doc_id: str) -> None:
        """
        Delete structured rows and assets for a document.
        Use before re-ingesting the same doc_id to avoid duplicates/stale rows.

        IMPORTANT:
        - Keep provenance rows in `doc_sources`
        - Keep `url_cache`
        Those belong to source/history management rather than transient parsed output.
        """
        with self.connect() as conn:
            conn.execute("DELETE FROM budget_rows WHERE tenant_id=? AND doc_id=?", (tenant_id, doc_id))
            conn.execute("DELETE FROM assets WHERE tenant_id=? AND doc_id=?", (tenant_id, doc_id))
            conn.execute("DELETE FROM kb_elements WHERE tenant_id=? AND doc_id=?", (tenant_id, doc_id))
            conn.execute("DELETE FROM kb_embeddings WHERE tenant_id=? AND doc_id=?", (tenant_id, doc_id))
            conn.commit()
