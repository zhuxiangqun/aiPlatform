...
- **向下依赖**：只能依赖 `aiPlat-core` 的 CoreFacade（唯一入口）。KNOWN_DEBT: kb/ 模块有 ~15 处直接 `from core.apps.*` import（文档分类/LLM 客户端/摘要器等，遗留未迁移），待通过 CoreFacade 统一暴露后消除。`from core.harness.*` 导入已全部消除（2026-05）。api/routers/ 中 1 处 Permission enum 类型导入保留。
- **禁止直接依赖** `aiPlat-infra`
