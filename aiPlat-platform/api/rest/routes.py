...
def _resolve_identity(request: Request) -> Identity:
    ...
    try:
        ...
        pass  # python-jose not installed  # noqa: optional-dependency
...
async def kb_ingest_url(request: Request):
    ...
    try:
        ...
        logging.getLogger(__name__).debug('kb_ingest_url failed', exc_info=True)
    return result
...
async def kb_reingest_document(doc_id: str, request: Request):
    ...
    try:
        ...
        logging.getLogger(__name__).debug('kb_reingest_document failed', exc_info=True)
    return core_resp
...
async def documents_ingest(request: Request):
    ...
    # v2.9: Auto-trigger ontology engine pipeline on newly ingested doc
    ...
    try:
        ...
        if doc_id:
            ...
            _asyncio.create_task(_auto_ontology_pipeline(doc_id, str(file_path), collection_id))
...
async def kb_watch_directory(request: Request):
    ...
    collection_id = str(body.get("collection_id", "default") or "default")

    recursive = bool(body.get("recursive", True))

    pattern = str(body.get("pattern", "*.md") or "*.md")

    kind = str(body.get("kind", "markdown") or "markdown")
    ...
    logging.getLogger("aiplat.platform").info(

        "Directory watch started: %s (recursive=%s, pattern=%s, watch_id=%s)",

        directory, recursive, pattern, watch_id)

    return {"status": "watching", "watch_id": watch_id, **result}
...
async def _auto_ontology_pipeline(doc_id: str, file_path: str, collection_id: str = "default"):

    u"""Background task: run ontology engine pipeline on a newly ingested document.

    

    Gated by AIPLAT_AUTO_ONTOLOGY_PIPELINE env var (default: true).

    Delegates through CoreFacade for layer boundary compliance.
    ...
    try:
        ...
        result = await auto_ontology_pipeline_for_doc(doc_id, file_path, collection_id)

        if result.get("status") not in ("disabled", "skipped"):

            logging.getLogger("platform.routes").info(

                "auto_ontology_pipeline: doc=%s domain=%s instances=%s",

                doc_id, result.get("domain", "?"), result.get("instances_created", 0))
...
# ── Ontology Editor module (v2.10) ──

from apps.ontology_editor.api.router import router as ontology_editor_router  # noqa: E402
