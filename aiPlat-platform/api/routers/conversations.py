...
def _init_platform_runtime() -> None:
    ...
    from core.api.core_facade import AgentDiscovery, AgentLoader, AgentRegistry, get_agent_registry  # P0-B4: 统一入口
    ...
    for da in discovered:
        ...
        if agent:
            agent_reg.register(
                da.agent_id,
                agent,
                {"model": da.model or best_model_for_purpose("chat")},
                metadata=da,
                skills=da.skills or [],
                tools=da.tools or [],
    ...
    from core.api.core_facade import set_kernel_runtime  # v2.5: canonical path
    from core.api.core_facade import get_harness  # v2.5: canonical path
