...
- [3. 侧边栏导航](#3-侧边栏导航)
- [4. 系统入口页面](#4-系统入口页面)
- [5. 基础设施层 (Infra)](#5-基础设施层-infra)
- [6. 核心能力层 (Core)](#6-核心能力层-core)
- [7. 提示词工程 (Prompts)](#7-提示词工程-prompts)
- [8. 应用能力层 (Workspace)](#8-应用能力层-workspace)
- [9. 平台服务层 (Platform)](#9-平台服务层-platform)
- [10. 应用接入层 (App)](#10-应用接入层-app)
- [11. 价值中心 (Value Center)](#11-价值中心-value-center)
- [12. 终端工作台 (User Workbench)](#12-终端工作台-user-workbench)
- [13. 诊断中心 (Diagnostics)](#13-诊断中心-diagnostics)
- [14. 典型操作流程](#14-典型操作流程)
...
侧边栏按功能分组，每组包含若干菜单项。左侧图标 + 文字标签，点击进入对应页面。

### 3.1 独立菜单项（仅 admin 可见）

| 图标 | 名称 | 路由 | 说明 |
|------|------|------|------|
| Activity | 系统概览 | `/system-overview` | 全局健康仪表盘 |
| Bell | 告警中心 | `/alerts` | 系统告警管理 |
| Rocket | 版本管理 | `/releases` | 系统版本发布记录 |
| Activity | 诊断中心 | `/diagnostics` | 综合诊断入口 |
| Activity | 修复中心 | `/diagnostics/repairs` | 一键修复问题 |
| BarChart3 | Agent 评估 | `/diagnostics/eval` | Agent 质量评分 |
| Activity | 系统图谱 | `/system-graph` | 架构可视化 |
| Settings | 初始化向导 | `/onboarding` | 新系统引导配置 |

### 3.2 分组菜单

| 组名 | 可见角色 | 菜单数量 | 说明 |
|------|---------|:---:|------|
| infra | admin, developer | 9 | 基础设施：节点、模型、微调、存储、网络、监控 |
| core | admin, developer | 8 | 核心能力：Agent、Skill、Tool、MCP、Memory |
| prompts | admin | 1 | 提示词工程模板 |
| workspace | admin, developer | 9 | 应用能力：Agent/Skill/Tool 库、商城、团队 |
| platform | admin | 5 | 平台服务：知识库、网关、认证、多租户 |
| app | admin, user | 6 | 应用接入：渠道、会话、构建、部署 |
| value | admin, developer, business | 6 | 价值中心：看板、KPI、目标、策略 |
| user | 全部 | 1 | 工作台 |
...
### 7.1 Agent 库（`/workspace/agents`）

**可见角色**：admin, developer

**功能**：用户工作区的 Agent 管理。与 Core Agent 管理的区别在于：这里是用户创建的应用级 Agent，允许完整 CRUD。

### 7.2 Skill 库（`/workspace/skills`）

**可见角色**：admin, developer

**功能**：用户工作区 Skill 管理。支持自定义 Skill 的创建、lint 检查、执行。

### 7.3 Tool 库（`/workspace/tools`）

**可见角色**：admin, developer

**功能**：用户工作区 Tool 注册和管理。

### 7.4 Workflow 库（`/core/workflows`）
