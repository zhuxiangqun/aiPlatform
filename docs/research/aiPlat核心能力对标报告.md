...
| 多渠道/接口 | ★★★★（Web + **10 渠道** + ACP/A2A） | ★★★（CLI/IDE/CI） | ★★★（ACP + SDK + Web） | ★★★★★（20+ IM Gateway） | ★★★★（app-server JSON-RPC stdio/WS + exec + TS/Python SDK） | ★★★★★（Gateway 22 平台 + CLI/TUI + ACP） |
