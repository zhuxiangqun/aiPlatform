...
| 29 | 生成 app 运行时端点 **`/platform/builder/projects/{id}/runtime*`**（生成物侧接线，2026-08-27）：`POST .../runtime/launch`（detect 入口 → daemon_jobs 托管启动，派生端口 18000-18999 + 127.0.0.1 绑定）、`GET .../runtime`（daemon job 存活 + 端口 + 入口类型）、`POST .../runtime/stop`（kill 会话组 + 清理记录）、`POST .../runtime/smoke`（detect→launch→HTTP 健康探测→报告，keep_alive 可选）；**运行范围限制在 `AIPLAT_HOME/apps/{project_id}/current` 白名单入口（FastAPI/Flask/Node/静态页 http.server），不执行任意命令**；`POST /projects/{id}/test` 的 e2e_smoke 从"目录存在"升级为真实冒烟 | `aiPlat-platform/builder/app_runtime.py`（detect_runtime/launch/health_check/stop/smoke_test）+ `aiPlat-platform/api/routers/builder.py`（4 端点）+ `aiPlat-platform/builder/builder_project_service.py`（runtime_* + run_tests 升级） | 生成 app 无运行能力 → 无法自动测试（e2e_smoke 假通过），生成物"死产物" |
