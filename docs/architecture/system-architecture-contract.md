...
| 32 | **用户可见输出风格 overlay**（2026-09，`adhd_v2`）：`output_style=adhd` 为 opt-in；经 `output_style.py` 尾部 overlay 注入（不改 system prompt 前缀，遵守 Prompt Cache 约束）；硬约束——禁止编造 ETA、高风险旁支须升级、`list_cap` 仅呈现截断不删工具事实；不改写 tool/stack/Clarify JSON；`STYLE_VERSION=adhd_v2` 与 seed `SKILL.md` metadata 对齐 | `aiPlat-core/core/harness/utils/output_style.py` + `workspace_seeds/skills/output_style_adhd/SKILL.md` + `core/tests/unit/test_harness/test_output_style.py` | 字面规则伤害（假 ETA / 漏报高风险 / 截断当删除） |
| 33 | **企业 AI 网关选型验收**（2026-09）：立项须书面答选型三问（管什么对象 / 哪一跳出域 / 控制面归谁）；三类失效模式（先出域再读 / 凭据不收敛 / 控制面在云）映射到模块时禁止把 hop/conformance 误标为 MCP/安全/网络网关；网络层私有出口**不在本仓库交付** | `docs/architecture/enterprise_deployment.md` + `private-control-plane.md` | 买在错层 / 对外材料夸大品类覆盖 |
