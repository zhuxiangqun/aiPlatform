...
| 32 | **媒体 true_test PRD 词表对齐**（2026-09）：`media_skill_handlers` 对 FR 考卷输出 `status=queued`（创建）/`error_code=DOWNLOAD_FAILED`/`source_type=url\|upload`/`skipped_reason=NO_AUDIO_TRACK\|NO_SOFT_SUBTITLE_TRACK`/`start_ms`/`end_ms`，以及报告顶层 `transcription`/`vision`；`report_json_export` 在 `skipped_stages` 时输出 `status=completed_with_skips` + `skip_reason` 并清空对应列表；`true_test_runtime` 同义词表兜底；**Factory「一键修复」不改平台 handler 词表**（仅 remap 技能名 / regenerate regenerate）——媒体测真失败须修 harness handler 或重跑测真 | `media_skill_handlers.py` + `media_ops.new_task_id` + `true_test_runtime._CONTAINS_SYNONYMS` + `test_media_skill_handlers_vocab.py` | VideoSense 等媒体生成物 50% 假失败 / 一键修复空转 |
