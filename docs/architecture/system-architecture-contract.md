...
| 29 | 生成 app 运行时端点 **`/platform/builder/projects/{id}/runtime*`**（生成物侧接线，2026-08-27）：`POST .../runtime/launch`（detect 入口 → daemon_jobs 托管启动，派生端口 18000-18999 + 127.0.0.1 绑定）、`GET .../runtime`（daemon job 存活 + 端口 + 入口类型）、`POST .../runtime/stop`（kill 会话组 + 清理记录）、`POST .../runtime/smoke`（detect→launch→HTTP 健康探测→报告，keep_alive 可选）、`POST .../runtime/real-tests`（**测试经理真实测试**：递归发现生成物测试用例 backend/tests/ 等任意层级 test_*.py → 可写临时目录跑 pytest（装依赖 + PYTHONPATH + conftest）→ test_report（header/meta/test_results/bug_summary，对齐 test_executor；失败分类 env/配置/实现 + suggested_fix））、`POST .../runtime/auto-repair`（**自动修复闭环**：真实测试失败 → LLM（经 CoreFacade llm_generate）按测试输出修复生成代码 → 临时区验证 → 改进才写回部署目录（_sync_repair_writeback，路径段匹配防逃逸）→ 重跑，最多 max_rounds 轮）；**运行范围限制在 `AIPLAT_HOME/apps/{project_id}/current` 白名单入口（FastAPI/Flask/Node/静态页 http.server），不执行任意命令；LLM 修复仅作用于部署目录内生成代码（abspath 前缀校验防路径逃逸）**；`POST /projects/{id}/test` 的 e2e_smoke 从"目录存在"升级为真实冒烟 + repo_tests 从"仅根目录 tests/"升级为递归真实测试 | `aiPlat-platform/builder/app_runtime.py`（detect_runtime/launch/health_check/stop/smoke_test/real_tests/auto_repair）+ `aiPlat-platform/api/routers/builder.py`（6 端点）+ `aiPlat-platform/builder/builder_project_service.py`（runtime_* + run_tests 升级） | 生成 app 无运行能力 → 无法自动测试（e2e_smoke 假通过），生成物"死产物" |
