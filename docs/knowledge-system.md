...
> 最后更新: 2026-08-11 · 原理优先，细节后置 · API/SQL/操作见附录

---

## 一、知识系统要解决什么问题

企业的知识散布在 PDF、Word、HTML、视频和数据库中。传统 RAG 面临三个核心挑战：

**1. 如何将异构资料转化为可检索的结构化知识？**
解决方案：一条 5 阶段管线。资料摄入→本体提取→向量索引→Wiki 合成→多路检索。
纯文本检索只能匹配关键词，无法理解「这篇 PDF 第 3 页的表格」和「那篇 Word 的第三章」讨论的是同一个概念。

**2. 如何让 RAG 检索到最相关的知识，而不是被无关内容淹没？**
解决方案：三大存储体系各司其职。向量 KB 存原始片段（搜原材料），GraphIndex 存实体/关系（搜结构化知识），LLM Wiki 存百科页面（搜人类可读知识）。检索时三路并行，路由到正确域过滤噪音。

**3. 如何发现知识缺口并自动补充？**
解决方案：质量闭环。HallucinationTracker 检测不忠实话→CandidatePool 积累候选缺口→ActiveSynthesis 自动生成补充页面→回写 Wiki。N≥3 次同一缺口才触发，避免误报。
...
│ 原始文档片段   │  知识结构图       │  人类可读百科页面              │
│ (SQLite)      │  (SQLite)        │  (Markdown 文件)              │
├───────────────┼──────────────────┼──────────────────────────────┤
│ 存什么        │ 存什么            │ 存什么                        │
│ 解析后的文本   │ 实体节点+关系边    │ 实体页+合成页+矛盾页           │
│ 片段+向量      │ +超边            │ +知识原子                     │
├───────────────┼──────────────────┼──────────────────────────────┤
│ 谁写          │ 谁写              │ 谁写                          │
│ 文档入库管线    │ 本体引擎 Step 10  │ KnowledgeSynthesizer         │
│ (不需要本体引擎) │ (提取实体+关系)    │ (从 GraphIndex 合成)          │
├───────────────┼──────────────────┼──────────────────────────────┤
│ 谁读          │ 谁读              │ 谁读                          │
│ SqliteEmbedding│ BFS 图遍历       │ WikiPageRetriever             │
│ Retriever     │                  │                              │
├───────────────┼──────────────────┼──────────────────────────────┤
│ 核心原则       │                  │                              │
│ 本体引擎不修   │                  │                              │
...
> 关于 TripleStore（将 5 个图连接为统一知识网络）请参见 §四·本体层。
...
用户查询
  │
  ├── [1] 资料摄入: DocumentParser→StructuredChunk→向量KB（零LLM，秒级可用）
  │
  ├── [2] 本体提取: OntologyEngine→实体/关系→GraphIndex（LLM异步，后台运行）
  │
  ├── [3] Wiki 合成: KnowledgeSynthesizer→Wiki 页面（从GraphIndex生成可读百科）
  │
  ├── [4] 知识检索: DomainRouter→三路并行→CRAG回退→RRF融合→答案
  │       Wiki(语义) + KB向量 + FTS5(关键词)
  │
  └── [5] 质量闭环: HallucinationTracker→ActiveSynthesis→回写Wiki
          不忠实话→候选缺口→自动补知识
```
...
| 阶段 | 组件 | 输入 | 输出 | LLM？ |
|:---:|------|------|------|:---:|
| ① 摄入 | DocumentParser | PDF/Word/HTML/MD | StructuredChunk | 否 |
| ② 提取 | OntologyEngine | StructuredChunk + 域YAML | 实体/关系→GraphIndex | 部分（PropertyExtractor） |
| ③ 合成 | KnowledgeSynthesizer | GraphIndex 实体/关系 | Wiki 页面 | ActiveSynthesis可选 |
| ④ 检索 | DomainRouter+CRAG+RRF | 用户查询 | 带来源的答案 | HyDE level 3 |
| ⑤ 闭环 | HallucinationTracker+ActiveSynthesis | ④的输出+用户信号 | 知识缺口→自动补充 | ActiveSynthesis |

### 异步最终一致性

引擎采用异步最终一致性模型：
- ClassMapper（零 LLM）打粗粒度标签后，**先行落地基础向量索引**，保证检索立即可用
- PropertyExtractor（LLM）+ GraphBuild 作为**增强管线异步运行**
- 管线崩溃/超时时，**基础索引仍可服务**，RAG 不会完全不可用
- 增强完成后覆盖更新向量 + 更新 Wiki 页面
...
**资料上传后立即创建基础索引（零 LLM，秒级可用）**。本体增强（LLM 提取实体/关系）在后台异步运行——二者独立，互不阻塞。
...
| 格式 | 解析器 | 说明 |
|------|--------|------|
| PDF | DocumentParser (MinerU / Tesseract) | 表格保留为 StructuredTable |
| DOCX/PPTX | DocumentParser | 保留标题层级 |
| HTML | DocumentParser (BeautifulSoup) | 保留结构 |
| Markdown | DocumentParser (内置) | 保留 heading 路径 |
| TXT | DocumentParser (内置) | 纯文本 |
| 图片 | DocumentParser (OCR) | 图像提取文本 |
| 视频 | DocumentParser (Whisper) | 转写+帧提取 |

外部数据源通过 DataSource 抽象层连接（SQL/API/CSV/Excel），不移动数据。

### 3.4 数据存储

存入 `kb_elements` 表（文本片段 + 向量），`kb_elements_fts` 表（全文索引）。DomainRouter 根据内容自动分类到对应域。

> 完整表结构和接入方式参见[附录 B](#附录-b存储表结构)。
...
本体层是五阶段管线中**承上启下的核心连接组织**。它从向量KB读取文档片段，提取实体/关系写入 GraphIndex，再经由 KnowledgeSynthesizer 生成 LLM Wiki 页面——最终这些 Wiki 页面成为 RAG 检索的主要数据源。

```
┌──────────────────────────────────────────────────────────┐
│                                                          │
│  向量KB                  GraphIndex           LLM Wiki    │
│  原始文档片段  ──提取──→  实体+关系  ──合成──→  百科页面   │
│  (不用本体引擎)          (本体引擎 Step10)    (Step11)     │
│        │                      ▲                  │        │
│        │                      │                  │        │
│        └── 引擎读取文档 ──────┘                  │        │
│                                                  │        │
...
│  RAG 检索（三路并行）                                      │
│  Wiki语义 + KB向量 + FTS5关键词                            │
│  ← DomainRouter 域路由 + CRAG回退 →                        │
└──────────────────────────────────────────────────────────┘
```

**四个核心关系**：

1. **向量KB→本体**：本体引擎从 `kb_elements` 表中读取文档片段，提取实体和关系写入 GraphIndex。向量KB 是「原材料仓库」，本体是「提炼后的知识模型」——二者独立运行，向量KB 不需要本体也能提供基础检索。
2. **本体→LLM Wiki**：KnowledgeSynthesizer 读取 GraphIndex 中的实体/关系，生成结构化 Wiki 页面。一个实体可对应多个页面：实体页（基本描述）、合成页（跨文档整合）、矛盾页（不一致检测）。
3. **本体→RAG 检索**：三重注入——DomainRouter 用域 YAML 定义的类标签路由查询到正确域、WikiPageRetriever 搜索本体引擎生成的 Wiki 页面、GraphIndex 遍历将实体关系作为检索关键词增强。
4. **本体→TripleStore**：5 个独立图通过本体定义的 URN 命名空间和 SPO 三元组连接到统一知识网络——使「删除 Tool X 影响哪些 Wiki 页面？」等跨域查询成为可能。

### 4.2 本体是什么

本体（Ontology）是知识的**结构化模型**，定义了：
- **类（Class）**：有什么类型的实体（如论文、技术、人物）
- **关系（Property）**：实体之间如何关联（如论文→使用了→技术）
- **状态机（State）**：实体如何演变（如新兴→成熟→淘汰）

每个域（Domain）有独立的本体定义文件 `~/.aiplat/ontologies/{domain_id}.yaml`。

### 4.3 引擎管线（13 步）

```
Phase 1（并行，无 LLM）:  Classify → Table Context
Phase 2（并行，LLM）:      PropertyExtractor  ← 本体建模的核心 LLM 调用
Phase 3（串行，确定性）:     Validate → Dedup → Build → SourceTrace
    → EntityResolver → Indicators → StateMachine → Reviews
...
域是知识系统的**租户级隔离单元**。DomainRouter 通过 3 层级联自动分类查询归属：

| 层级 | 机制 | 延迟 | 命中率 |
|:---:|------|:---:|:---:|
| T1 | 标签倒排索引 | <1ms | ~60% |
| T2 | 加权域向量余弦相似 | ~50ms | ~30% |
| T3 | LLM 二分类 | ~300ms | ~10% |

新域修改 YAML 后无需重启，DomainRouter 自动热加载。

域配置 YAML 示例：

```yaml
# ~/.aiplat/ontologies/ai-knowledge.yaml
name: AI知识
classes:
  AITechnique:
    label: AI方法
    ...
    states:
      ...
      transitions:
        - from: emerging
          to: established
          trigger: { type: relation_count, relation: implemented_by, threshold: 3 }
```

### 4.5 统一知识图（TripleStore）

系统有 5 套独立图结构，通过 TripleStore 的 SPO 三元组统一连接：

| 桥接 | 说明 | 状态 |
|------|------|:---:|
| Agent↔Skill/Tool/Model | 能力依赖 | ✅ |
| Skill↔Syscall | 底层调用 | ✅ |
| Wiki→Agent | 知识被谁引用 | ✅ V1 |
| Pipeline→Agent | 运行时绑定 | ✅ |
| KB文档→Wiki页面 | 文档产出 | ✅ |

> Agent/Skill/Tool 在此作为 TripleStore 知识图的合法节点出现——这是跨域查询的基础。能力实体的详细定义参见[能力剖面文档](architecture/capability-profile.md)。

> 详细 API 参见[附录 A](#附录-aapi-端点速查)。完整域管理 CRUD 操作见[附录 C](#附录-c本体操作手册)。

---

## 五、检索层：如何找到最相关的知识

### 5.1 检索策略：多路并行 + CRAG 回退

```
用户查询
  │
  ├── [1] DomainRouter.classify(query) → 确定查询归属域
  ├── [2] map_query_to_ontology(query, domain_id) → 本体感知检索
  │
  ├── 路径 A: 本体优先检索（WikiPageRetriever + 域过滤）
  ├── 路径 B: GraphIndex 遍历 → 增强检索词
  └── 路径 C: KB 向量检索（SqliteEmbeddingRetriever）
         │
         ▼
    RRF 融合 + 去重（Wiki 权重 1.1×）→ Top-K
         │
         ▼
    CRAG 3 级回退检查：
      Level 1: 结果 > 1000 字 → 直接使用
      Level 2: 结果 < 1000 字 → FTS5 关键词检索
      Level 3: 结果 < 50 字 → HyDE（LLM 生成假设答案 → 重新检索）
```

### 5.2 检索原则

1. **本体驱动**：优先使用域本体过滤检索范围，避免跨域噪音
2. **多路融合**：Wiki（语义）、FTS5（关键词）、GraphIndex（实体验证）三路并行
3. **CRAG 回退**：结果不足时自动降级，保证至少有一条结果
4. **HyDE 兜底**：当所有路径失败时，LLM 生成假设答案反向检索
...
```
用户查询 → RAG 检索 → LLM 生成答案
                          │
                          ▼
              HallucinationTracker（检测不忠实话）
                          │
                          ▼
              FeedbackRadar（检测用户隐式信号）
                          │
                          ▼
              CandidatePool（积累候选知识缺口）
                          │
                    N ≥ 3 时触发
                          │
                          ▼
              ActiveSynthesis（自动生成补充 Wiki 页面）
                          │
                          ▼
              回写 Wiki → 下次检索可获取 → 闭环完成
```
...
| 组件 | 作用 | 触发条件 |
|------|------|------|
| HallucinationTracker | 检测 LLM 回答是否忠实于检索到的知识 | 每次 RAG 回答后 |
| FeedbackRadar | 检测用户隐式不满信号（快速关闭/重复提问/差评） | 实时监控用户行为 |
| CandidatePool | 积累候选知识缺口（同一缺口出现 3 次才触发） | HallucinationTracker + FeedbackRadar 输出 |
| ActiveSynthesis | LLM 自动生成补充 Wiki 页面填补缺口 | CandidatePool 中 N ≥ 3 |
...
| 指标 | 含义 | 告警阈值 |
|------|------|:---:|
| 管线延迟 P95 | OntologyEngine 端到端耗时 | > 60s |
| 检索召回 Recall@10 | RAG 检索命中率 | < 85% |
| 状态转换准确率 | StateMachine 预测准确率 | < 80% |
| Hallucination 率 | 检测到不忠实话的比例 | > 10% |

### 8.2 诊断入口

- 管线诊断：`GET /api/core/wiki/pipeline/diagnose/{run_id}`
- 本体健康检查：`check_ontology_health()`（孤儿实体/废弃模型/孤立 Wiki）
- TripleStore 统计：`GET /api/core/ontology/stats`
- 知识健康仪表盘：管理端 → 诊断 → 知识健康

> 完整运维命令见[附录 A](#附录-aapi-端点速查)。
...
**Q: 引擎跑了很久没结果？**
A: 检查引擎日志（`~/.aiplat/ontology_engine.log`）。Phase 2 的 PropertyExtractor 是 LLM 调用，大文档可能超时。可通过 `POST /api/core/ontology/domains/{id}/build-instances` 重试，已成功处理的文档会跳过。

**Q: 知识库检索不到刚上传的文档？**
A: 检查引擎是否已完成对该文档的处理。文档上传后立即创建基础索引（零 LLM），但本体增强（实体提取）在后台异步运行。二者独立，不影响检索可用性。

**Q: 为什么需要本体模型？**
A: 没有本体 = 纯关键词匹配（"AI" 匹配到 "air"、"chair"）。本体告诉系统 "AI" 是一个技术术语、归属于 "AI知识" 域、与 "论文" 和 "应用场景" 相关——从而过滤掉不相关的结果。

**Q: 域之间有冲突怎么办？**
A: 域隔离设计保证不同域的本体互不干扰。跨域查询通过 TripleStore 的 BFS 遍历实现，不要求域之间有共享命名。

**Q: Pipeline 崩溃后数据会丢失吗？**
A: 不会。引擎采用异步最终一致性模型——基础索引先行落地，增强处理在后台运行。即使增强崩溃，基础检索仍可用。通过重跑引擎即可恢复。

**Q: 如何新增一个域？**
A: 创建 `~/.aiplat/ontologies/{domain_id}.yaml`，在 `registry.json` 中注册。无需重启，DomainRouter 自动热加载。详见[附录 C](#附录-c本体操作手册)。
...
| 端点 | 方法 | 说明 |
|------|:---:|------|
| `/api/core/wiki/ingest` | POST | 文档摄入（PDF/Word/HTML/URL） |
| `/api/core/wiki/search` | GET | Wiki 语义搜索 |
| `/api/core/ontology/domains` | GET/POST | 创建/列出域 |
| `/api/core/ontology/domains/{id}` | PUT/DELETE | 编辑/删除域（删除自动快照） |
| `/api/core/ontology/domains/{id}/classes` | POST | 添加类定义 |
| `/api/core/ontology/domains/{id}/properties` | POST | 添加关系定义 |
| `/api/core/ontology/domains/{id}/build-instances` | POST | 触发引擎管线（全量重跑） |
| `/api/core/ontology/domains/{id}/verify` | POST | 验证分类覆盖率 |
| `/api/core/ontology/domains/generate` | POST | AI 自动生成本体 |
| `/api/core/ontology/engine/snapshot/{id}` | POST | 创建图快照 |
| `/api/core/ontology/engine/snapshots/{id}` | GET | 列出快照 |
| `/api/core/ontology/engine/snapshot/{id}/restore` | POST | 恢复快照 |
| `/api/core/ontology/engine/graph-stats/{id}` | GET | 图统计 |
| `/api/core/ontology/engine/recommend/{id}` | GET | 推荐阅读 |
| `/api/core/ontology/engine/simulate-scenarios` | POST | 场景推演沙箱 |
| `/api/core/ontology/query` | POST | 跨域本体查询（upstream/downstream） |
| `/api/core/ontology/impact/{urn}` | GET | 上下游影响分析 |
| `/api/core/ontology/stats` | GET | TripleStore 统计 |
| `/api/core/ontology/sdk/{id}` | GET | 生成域 SDK 代码 |
| `/api/core/wiki/pipeline/diagnose/{id}` | GET | 管线运行诊断 |
| `/api/platform/apps/fde/ingest` | POST | FDE 平台文档上传 |

---

## 附录 B：存储表结构

### B.1 向量 KB（`~/.aiplat/kb/tenants/{id}/kb.sqlite3`）

```sql
CREATE TABLE documents (
  doc_id TEXT PRIMARY KEY,
  collection_id TEXT, source_uri TEXT,
  kind TEXT, status TEXT, meta_json TEXT
);

CREATE TABLE kb_elements (
  tenant_id TEXT, element_id TEXT, doc_id TEXT,
  type TEXT, page_idx INTEGER, text TEXT, meta_json TEXT
);

CREATE TABLE kb_embeddings (
  embedding_id TEXT, element_id TEXT,
  vector_json TEXT, model TEXT, dim INTEGER
);

CREATE VIRTUAL TABLE kb_elements_fts USING fts5(text, content=kb_elements);
```

### B.2 GraphIndex（`~/.aiplat/graph/{domain}.db`）

每个域有独立的 SQLite 图数据库，存储实体节点、关系边和超边。

### B.3 TripleStore（`~/.aiplat/ontology_triples.sqlite3`）
...
当前谓词类型：`uses_skill`、`uses_tool`、`uses_model`、`calls_syscall`、`requires_permission`、`depends_on`、`depends_on_kb`、`produces_artifact`、`member_of_phase`、`used_by_agent`。
...
> ✅ 删除时系统自动创建 `pre-delete-{domain_id}` 快照，无需手动操作。
...
> 本体操作手册现统一归并到该文档的「十一、本体操作手册」章节中。