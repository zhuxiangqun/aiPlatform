...
- **落地记录（2026-08-24 五十五轮）**：P0-a stdio JSON-RPC 持久内核（Codex-Harness 借鉴，对标 app-server）——`core/acp/stdio_server.py`：`StdioKernel`（JSON-RPC 2.0 over stdio JSONL）+ `handle_request`（initialize/shutdown/thread/start|status|events|resume|approve|reject|rollback|cancel）+ `_event_loop`（逐行读 stdin→分发→写 stdout，EOF 退出）+ 背压 `-32001`（并发超限指数退避语义）。映射：`thread/start`→`create_pipeline_session().start`（`CoreFacade`）、`thread/approve|reject`→`session.approve|reject`（HITL 审批协议化）、`thread/events`→`PipelineRunStore.list_run_events`（run_events 事件流经 `item.event` 推送）、`thread/cancel`→`cancel_pipeline`。入口：独立进程 `python -m core.acp.stdio_server` + `CoreFacade.start_stdio_kernel` + server 启动 `AIPLAT_STDIO_KERNEL=1` env 门控。契约：stdio 协议为薄映射层（零新增业务逻辑，全部委托已有门面）；run_id/trace 语义不变（thread 是 run 的协议别名，run_events 仍是唯一事件真相源）。测试：`test_stdio_kernel.py` 13 例（JSON-RPC 帧/方法分发/错误信封/背压/Thread 生命周期/审批流）+ 进程级 smoke（initialize+shutdown 往返）。
- **落地记录（2026-08-24 五十六轮）**：P0-b 竞品会话/记忆导入（Codex-Harness 借鉴，对标 external_agent_config_migration）——`core/harness/memory/import_claude_sessions.py`：`parse_claude_session`（Claude JSONL user/assistant 配对、system-reminder 跳过、text/block-list content 提取、孤儿 user 丢弃）+ `find_claude_sessions`（~/.claude/projects|transcripts 递归，上限 200）+ `import_claude_sessions`（→ `MemoryManager.save_interaction`，`session_id=claude:{session_id}` + `source_tag=claude_import` + `provenance=claude:{session_id}:{file}` 防投毒溯源，单会话失败 best-effort 不阻断）。接线：`CoreFacade.import_claude_memories` + platform `POST /platform/memory/import` + `GET /platform/memory/import/status`（require_auth，经 CoreFacade）。契约：导入为只读消费（不改 Claude 源文件，对齐 Codex"源端不动一字节"）；记忆写入统一走 MemoryManager（不直写 SQLite）。测试：`test_import_claude_sessions.py` 10 例（解析配对/噪音跳过/坏行容错/查找/导入集成/失败隔离/空会话跳过）。
- **落地记录（2026-08-24 五十七轮）**：P1 SDK stdio 内核客户端（Codex-Harness 借鉴，G17 闭环）——`aiplat-sdk/aiplat/stdio.py`：`StdioKernelClient`（spawn `python -m core.acp.stdio_server` + JSON-RPC over stdio：initialize/shutdown + thread/start|status|events|resume|approve|reject|rollback|cancel + `stream_events` 轮询式流式监听 + 错误信封/超时/EOF 处理）+ 可注入 transport（测试）；`aiplat.__init__` 导出 `StdioKernelClient`/`StdioKernelError`，README 示例含 stdio 用法。契约：SDK 纯客户端零业务逻辑；与 P0-a 内核协议一一对应（thread 是 run 的协议别名，run_id/trace 语义不变）；`stream_events` 按 run_events seq 增量拉取（对齐 item.event 事件流）。测试：`test_stdio_client.py` 8 例（协议层 7 + 真实内核集成 1，全部通过）。
- **落地记录（2026-08-24 五十八轮）**：P1 OS 原生沙箱执行器（Codex-Harness 借鉴收官，G19 闭环）——`core/harness/infrastructure/os_sandbox.py`：`detect_sandbox_mode`（bwrap/seatbelt/无探测 + `AIPLAT_SANDBOX` 开关）+ `build_os_sandbox_cmd`（bwrap：`--ro-bind` 系统路径（/usr /lib /bin /etc/ssl 等只读）+ `--bind` 工作区/tmp 可写 + `--unshare-net` 默认网络隔离 + `--die-with-parent`；seatbelt：`sandbox-exec -p` deny-default profile）+ `sandbox_env_ready` 诊断；fail-open fallback（无 bwrap/seatbelt 二进制或包装失败返回原命令）。接线：`StageSandbox.run` 包装 worker 子进程（`sandbox.py:103`）+ `CoreFacade.get_os_sandbox_status`（诊断）。契约：OS 沙箱为可选加固层（setrlimit 进程级沙箱保持默认）；fail-open 与安全降级方案 B 哲学一致（决策不受基础设施故障影响）。测试：`test_os_sandbox.py` 10 例（模式探测/命令包装/fail-open/网络开关/seatbelt profile/诊断）。
- **落地记录（2026-08-24 五十九轮）**：渠道广度延伸二批（10→14，对齐 Hermes 22 平台）——`aiPlat-app/channels/adapters/` 新增 `signal.py`（signal-cli json：envelope/dataMessage 解析）+ `matrix.py`（Matrix client-server API：m.room.message/content/sender/room_id）+ `mattermost.py`（webhook：text/user_name/channel_name）+ `line.py`（LINE Messaging API：events/replyToken/source/message）；`ChannelType` 补 SIGNAL/MATRIX/MATTERMOST/LINE 枚举 + `ADAPTERS` 注册（3 内置 + 11 扩展 = 14）。契约：渠道适配为 app 层归一化职责（统一 `ChannelMessage`），`POST /channels/{id}/test` 经 `get_channel_adapter` 动态解析自动覆盖（零 platform 改动）；未知渠道 ValueError。测试：`test_cli_and_channels.py` 15 例（含 4 新适配器 parse/format 样例 + 14 渠道可解析）。
- **落地记录（2026-08-24 六十轮）**：前端 coding 场景（对标报告 §16.3 唯一 ⚠️未变项闭环）——新增 `FileCheckpoints.tsx`（`/core/checkpoints` 路由 + 侧边栏"文件 Checkpoint"菜单）：checkpoint 列表（session/路径过滤 + 刷新，`/platform/execution/file-checkpoints`）+ 查看内容（Modal 代码块渲染 `GET /{id}`）+ 恢复（confirm + `POST /{id}/restore`）。`checkpointApi` 定义于 services/coreApi.ts 并经 services/index.ts 统一出口（对齐管理端 §5.1）。后端能力（`list/get/restore_file_checkpoint` + RBAC）已完备（Hermes Layer 1 物理安全网），本次纯前端接入。验证：tsc 0 error + npm run build exit 0。
- **落地记录（2026-08-24 六十一轮）**：continuable 子代理编排（对标报告 §21.1 "continuable 编排仍缺"闭环，DSH send_message 对齐）——`coordinator.continue_execution(instance_id, message)`：复用 `execute_single` 创建的 agent（`SubagentInstance.agent_ref` 保留，conversational agent `_conversation_history` 天然多轮），向同一会话追加消息重执行；fail-loud（未知 instance/无 agent_ref → 明确错误）。`execute_single` 返回真实 instance key（`SubagentResult.instance_id` = `{session_id}:{name}`，替代原 `inproc:{name}` 占位）；`InProcessProvider.continuation` 委托 coordinator（`capabilities.continuation=True` 名实相符）；`coordinator.send_message` → in_process 续接成功。契约：continuable 为编排层能力，agent 上下文隔离在会话内，多轮续接由 conversational agent 天然支持。测试：`test_subagent_providers.py` 24 例（含 4 新增：真实 key 返回/续接复用同一 agent/未知 instance fail-loud/send_message 续接成功）。
- **落地记录（2026-08-24 六十二轮）**：模型 provider 生态广度（对标报告 §21.1 "家族数远少于 38"收窄，P2-A3 机制扩展）——`aiPlat-infra/config/providers.yaml` 新增 8 个 OpenAI 兼容 provider 家族：qwen(DashScope)/groq/mistral/cohere/cerebras/together/xai/novita（各带 env_key + base_url_env，复用 openai_compatible.py 零代码）。provider 总数 6→14（外部 API 13 + ollama local）；`ModelManager._api_provider_ids()` YAML 驱动自动发现 13 个 external（非 fallback 硬编码集）。契约：模型选择候选必须来自 infra 注册表（env 不再干预，对齐 §12 模型解析中心化）；新增 provider 零代码（仅 YAML）。测试：`test_model_selection.py` 16 例（含 2 新增生态广度防回归：新家族可发现 + providers.yaml ≥14 + external env_key 契约）。
- **落地记录（2026-08-24 六十三轮）**：渠道广度延伸三批（14→18，对齐 Hermes 22 平台）——`aiPlat-app/channels/adapters/` 新增 `qq.py`（QQ 机器人回调：event.message.content/chat_id/from_id）+ `reddit.py`（comment：data.name/author/subreddit/body）+ `github.py`（webhook：issue/comment/repository/sender，action 前缀标注）+ `sms.py`（Twilio 风格：From/To/Body/MessageSid）；`ChannelType` 补 QQ/REDDIT/GITHUB/SMS 枚举 + `ADAPTERS` 注册（3 内置 + 15 扩展 = 18）。契约：渠道适配为 app 层归一化职责（统一 `ChannelMessage`），`POST /channels/{id}/test` 经 `get_channel_adapter` 动态解析自动覆盖（零 platform 改动）；未知渠道 ValueError。测试：`test_cli_and_channels.py` 20 例（含 4 新适配器 parse/format 样例 + 18 渠道可解析）。
- **落地记录（2026-08-24 六十四轮）**：事件源纯度——Fork 会话 + 血缘（对标报告 §21 改进项⑥，DSH fork / Codex thread-fork 对齐）——`PipelineRunStore.fork_run_from_events(source_run_id, new_run_id, project_id, *, note)`：折叠源 run 事件（`replay_run_events`）→ 新 pipeline_runs 行继承分叉点（current_stage_idx / pass_rate / total_stages），phase 归位 executing（从分叉点继续）；`pipeline_forked` 事件（append-only，携带 parent_run_id + note + source_event_count + 继承的 current_stage_idx/pass_rate）写入新 run 事件日志——子 run 状态可纯从自身事件重建（`replay_run_events` 折叠 `pipeline_forked`）；`list_forked_runs(parent_run_id, limit)` 血缘查询（json_extract payload.parent_run_id）。接线：`POST /pipeline/pipelines/runs/{run_id}/fork`（body: new_run_id 必填 + project_id/note 可选，源无事件 404）+ `GET /pipeline/pipelines/runs/{run_id}/forks`（血缘列表）。契约：run_id 语义扩展为**可派生**——fork 生成新 run_id，父 run_id 记录于子事件（血缘可观测）；事件日志保持 append-only（fork 不写回父，源不被污染）；state+event 双写架构不变（快照仍为快速路径，事件仍为审计真相源）。测试：`test_pipeline_run_events.py` 9 例（含 3 新增：fork 继承状态+血缘+purity 断言、无事件源返回 None、多子血缘逆序/limit）。
