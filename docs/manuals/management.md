...
aiPlat 管理画面是一个四层架构的 AI 中台管理界面，覆盖基础设施、AI 引擎、平台服务和应用接入全栈管理。首页为**系统概览**仪表盘，左侧侧边栏按任务流分组（6 组）。右下角提供**角色切换器**用于不同身份视角切换。
...
侧边栏按任务流分为 6 个分组，每组包含若干菜单项。分组内用子区域标题分段，左侧图标 + 文字标签，点击进入对应页面。支持展开/折叠 + URL 自动匹配展开。
