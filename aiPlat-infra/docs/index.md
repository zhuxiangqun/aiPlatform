...
**职责（As-Is）**：提供基础设施能力的抽象与默认实现（如 MCP/DI/Observability 等），供上层（core/platform/app）按需使用。
...
> 注：是否“被 core 直接依赖”属于工程选择。当前仓库存在 `core/apps/mcp` 与 `infra/mcp` 双栈并存（详见 infra 的 MCP 文档），短期以 core 为主实现，infra 作为基础设施实现与参考。
